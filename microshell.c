...
{
	if (dup2(fd_in, STDIN_FILENO) < 0 && !close(fd_in) && !close(fd_pipe[0]) && !close(fd_pipe[1]))
		fatal(cmd, 1);
	if (find_next_pipe(tmp) && dup2(fd_pipe[1], STDOUT_FILENO) < 0 && !close(fd_in) && !close(fd_pipe[0]) && !close(fd_pipe[1]))
		fatal(cmd, 1);
...
int execute(char **cmd, char **env)
{
	...
	while (tmp)
	{
		if (pipe(fd_pipe) < 0 && !close(fd_in))
			fatal(cmd, 1);
		if ((pid = fork()) < 0 && !close(fd_in) && !close(fd_pipe[0]) && !close(fd_pipe[1]))
			fatal(cmd, 1);
		if (!pid)
